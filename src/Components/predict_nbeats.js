...
const PredictNBeats = () => {
  ...
  const handlePredict = async (skill) => {
    ...
    try {
      const response = await fetch('/forecast/', {
        ...
        body: JSON.stringify({ skill, forecast_years: years, mode: 'nbeats' })
      });
  ...
  // Deduplicate forecast by year (keep first occurrence)
  const dedupedForecast = forecast
    ? forecast.filter((item, idx, arr) => arr.findIndex(f => f.year === item.year) === idx)
    : null;

  const chartData = dedupedForecast
    ? {
        labels: dedupedForecast.map(item => item.year),
        datasets: [
          {
            label: 'Predicted Value',
            data: dedupedForecast.map(item => item.value),
            fill: false,
            borderColor: '#3498db',
          ...
          },
          {
            ...
            data: dedupedForecast.map(item => item.upper_ci),
            fill: '+1',
          ...
          },
          {
            ...
            data: dedupedForecast.map(item => item.lower_ci),
            fill: false,
