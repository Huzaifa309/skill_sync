...
const Profile = () => {
  ...
  const handleSkillSelect = (skill, type) => {
    const currentSkills = Array.isArray(formData[type]) 
      ? formData[type] 
      : (formData[type] || '').split(',').map(s => s.trim()).filter(s => s.length > 0);
          ...
          <>
            ...
            <div className="skill-input-container">
              ...
              <div className="selected-skills">
                {(Array.isArray(formData.skills) ? formData.skills : (formData.skills || '').split(',').map(s => s.trim()).filter(s => s.length > 0))
                  .map(skill => (
                ...
                <div className="skill-options">
                  {availableSkills
                    .filter(skill =>
                      ...
                      !(Array.isArray(formData.skills) ? formData.skills : (formData.skills || '').split(',').map(s => s.trim())).includes(skill) &&
                      !(Array.isArray(formData.skillsToAcquire) ? formData.skillsToAcquire : (formData.skillsToAcquire || '').split(',').map(s => s.trim())).includes(skill)
                    )
                    .slice(0, 10)
                    .map(skill => (
                      <div
            ...
            <div className="skill-input-container">
              ...
              />
              <div className="selected-skills">
                {(Array.isArray(formData.skillsToAcquire) ? formData.skillsToAcquire : (formData.skillsToAcquire || '').split(',').map(s => s.trim()).filter(s => s.length > 0))
                  .map(skill => (
                    <div key={skill} className="skill-tag">
                      {skill}
                      <button
